...
@Component
class HangupTheCallBuilder : BinotelRequestBuilder {
    override val requestType = BinotelRequestType.HANGUP_THE_CALL

    override fun build(request: Map<String, String>): BinotelCallRequest {
        val json = jacksonObjectMapper().writeValueAsString(request)
        return jacksonObjectMapper().readValue(json, HangupTheCall::class.java)
    }
}
